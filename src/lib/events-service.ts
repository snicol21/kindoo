...
export function clampImportTimes(input: AddEventInput): AddEventInput {
...
export function getDefaultEventTypeForRole(role: UserRole): EventType {
  return role === 'ward_manager' || role === 'ward_user' ? 'Ward' : 'Private';
}

export function normalizeEventType(value: unknown, fallbackEventType: EventType): EventType {
...
export function validateEventInput(input: AddEventInput): string | null {
...
/** Stricter validation for Facilities Lead HTTP API: email AND phone required. */
export function validateApiBookingInput(input: AddEventInput): string | null {
  const baseError = validateEventInput(input);
  if (baseError) return baseError;
  if (!input.email?.trim()) return 'Contact email is required.';
  if (!input.phone?.trim()) return 'Contact phone is required.';
  return null;
}
...
export async function cleanupOrphanContacts(contactIds: string[]) {
...
export async function createEventForAccess(
  access: AccessContext,
  input: AddEventInput,
  options?: { requireEmailAndPhone?: boolean }
): Promise<ActionResult<Event>> {
  try {
    const { userId, role, ward } = access;

    const normalizedInput = normalizeEventInput(input, getDefaultEventTypeForRole(role));
    const validationError = options?.requireEmailAndPhone
      ? validateApiBookingInput(normalizedInput)
      : validateEventInput(normalizedInput);
    if (validationError) {
    ...
    try {
      ...
      console.info('[createEventForAccess] SMS notification summary:', {
        actorUserId: userId,
      ...
      console.error('[createEventForAccess] Failed to send SMS notifications:', error);
    }
    ...
    console.error('[createEventForAccess] Error:', error);
    return {
      success: false,
...
function buildRoleScopedBuildingPredicate(
  access: AccessContext,
  building: Building
): SQL | undefined {
  const { userId, role, ward } = access;
  if (role === 'admin' || role === 'stake_manager') {
    return eq(events.building, building);
  }
  if (role === 'ward_manager') {
    return and(eq(events.building, building), eq(contacts.ward, ward));
  }
  return and(eq(events.building, building), eq(events.userId, userId), eq(contacts.ward, ward));
}

export async function listEventsForAccess(
  access: AccessContext,
  filters: ListEventsFilters
): Promise<ActionResult<EventWithCreator[]>> {
  try {
    if (!filters.building || !BUILDINGS.includes(filters.building)) {
      return { success: false, error: 'Invalid or missing building filter.', data: [] };
    }

    const predicates: SQL[] = [];
    const rolePredicate = buildRoleScopedBuildingPredicate(access, filters.building);
    if (rolePredicate) predicates.push(rolePredicate);

    if (filters.ward) {
      if (!WARDS.includes(filters.ward)) {
        return { success: false, error: 'Invalid ward filter.', data: [] };
      }
      predicates.push(eq(contacts.ward, filters.ward));
    }

    if (filters.dateFrom) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(filters.dateFrom)) {
        return { success: false, error: 'dateFrom must be YYYY-MM-DD.', data: [] };
      }
      predicates.push(gte(events.eventDate, filters.dateFrom));
    }

    if (filters.dateTo) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(filters.dateTo)) {
        return { success: false, error: 'dateTo must be YYYY-MM-DD.', data: [] };
      }
      predicates.push(lte(events.eventDate, filters.dateTo));
    }

    if (filters.eventId) {
      predicates.push(eq(events.id, filters.eventId.trim()));
    }
    ...
    console.error('[listEventsForAccess] Error:', error);
    return { success: false, error: 'Failed to fetch events.', data: [] };
  }
}

export async function deleteEventForAccess(
  access: AccessContext,
  eventId: string
  ...
  try {
    ...
    console.error('[deleteEventForAccess] Error:', error);
    return { success: false, error: 'Failed to delete event.' };
  }
}

export async function deleteEventsForAccess(
  access: AccessContext,
  eventIds: string[]
  ...
  try {
    ...
    console.error('[deleteEventsForAccess] Error:', error);
    return { success: false, error: 'Failed to delete events.' };
  }
}

export async function updateEventForAccess(
  access: AccessContext,
  ...
  try {
    ...
    console.error('[updateEventForAccess] Error:', error);
    return { success: false, error: 'Failed to update event.' };
  }
}

export async function importEventsForAccess(
  access: AccessContext,
  input: { events: AddEventInput[] }
  ...
  try {
    ...
    console.error('[importEventsForAccess] Error:', error);
    return { success: false, error: 'Failed to import events.' };
  }
}

export async function setKindooLicenseCreatedForAccess(
  access: AccessContext,
  input: {
    eventId: string;
    kindooLicenseCreated: boolean;
  ...
  try {
    ...
    console.error('[setKindooLicenseCreatedForAccess] Error:', error);
