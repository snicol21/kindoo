...
function getLogLineClassName(line: string) {
  if (
    ...
    line.includes(' run timeout/flow error ') ||
    line.includes(' failed:')
  ) {
    return 'text-rose-300';
  }
  if (
    line.includes('step:done') ||
    line.includes(' automation run completed') ||
    line.includes(' completed {')
  ) {
    return 'text-emerald-300';
  }
  if (line.includes('step:start') || line.includes(' navigating ') || line.includes(' opening ')) {
...
export function KindooLicenseDialog({
  ...
  return (
    <>
      ...
      <Dialog open={isRunLogOpen && !!hasRunLog} onOpenChange={setIsRunLogOpen}>
        <DialogContent className="flex max-h-[90vh] w-[95vw] max-w-5xl flex-col">
          <DialogHeader>
            <DialogTitle>Automation run log</DialogTitle>
            <DialogDescription>
              Full transcript for the selected Kindoo automation job.
            </DialogDescription>
          </DialogHeader>

          <div className="min-h-[40vh] flex-1 overflow-hidden rounded-md border border-slate-800 bg-slate-950">
            <div className="flex items-center justify-between border-b border-slate-800 bg-slate-900/90 px-3 py-2">
              ...
              <p className="font-mono text-[11px] text-slate-300">kindoo-worker.log</p>
            </div>

            <div className="relative h-full max-h-[62vh] overflow-auto bg-[radial-gradient(ellipse_at_top,rgba(45,212,191,0.08),transparent_50%)] p-4">
              <div className="pointer-events-none absolute inset-0 opacity-15 bg-[linear-gradient(to_bottom,transparent_50%,rgba(15,23,42,0.4)_50%)] bg-size-[100%_3px]" />
              <div className="relative space-y-1 font-mono text-[11px] leading-relaxed">
                {(latestRunLog ?? '').split('\n').map((line, index) => (
                  <div key={`${index}-${line.slice(0, 24)}`} className="flex gap-3">
                    ...
                    <span className={`${getLogLineClassName(line)} break-all`}>{line || ' '}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsRunLogOpen(false)}>
              Close logs
            </Button>
          </DialogFooter>
        </DialogContent>
