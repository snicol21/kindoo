...
export function UsersTable({
  ...
  const canSelfEditWard = currentUserRole === 'admin' || currentUserRole === 'stake_manager';
        ...
        <Table>
          ...
          <TableBody>
            {users.map((user) => {
              ...
              const canEdit = canManage || (user.id === currentUserId && canSelfEditWard);
                ...
                <TableRow key={user.id}>
                  ...
                  <TableCell className="text-right md:w-px md:whitespace-nowrap">
                    {user.id === currentUserId && (
                      <span className="mr-2 inline-flex rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">
                        You
                      </span>
                    )}
                    {canEdit || canDelete ? (
